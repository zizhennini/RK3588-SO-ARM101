...
| T15 | **主从臂姿态对不上**（"主臂比从臂高一点"） | 5 个身体关节的 `range_min/max` 是**扫出来**的，代入 `val = 编码器 + homing_offset` 后 homing **完全约掉** → 偏移只来自**扫过的物理行程**；而 **`wrist_roll` 的 range 是硬编码 0/4095**，homing 约不掉 → **按 ENTER 那一刻的转角就是它的零点**，两臂必须转到位才一致（官方 issue [#3193](https://github.com/huggingface/lerobot/issues/3193)）→ 用 `scripts/calibrate_so101.py compare` 量，别靠眼睛 |
